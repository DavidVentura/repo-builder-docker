...
ARG BASE_DIR
ADD ${BASE_DIR}/package-lock.json /tmp/package-lock.json
ADD ${BASE_DIR}/package.json /tmp/package.json
RUN cd /tmp && npm install
RUN mkdir -p /usr/src 

COPY ${BASE_DIR}/ /usr/src/app/
WORKDIR /usr/src/app/
...
RUN for artifact in ${ARTIFACTS}; do \
	...
	s4cmd --endpoint-url=http://ci.labs:9000 put --force $artifact s3://${BUCKET_NAME}/${TAG}/; \
	done
RUN curl -s http://david-dotopc.labs:8080/deploy/${REPO_NAME}/${TAG}
